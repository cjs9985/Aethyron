        ...
        plan.tasks.retain(|task| {
            ...
            for word in words {
                if word.starts_with("src/")
                    ...
                    || word.starts_with("tests\\")
                {
                    let path = word.trim_matches('`').trim_matches(',').trim_matches('.');

                    let normalized_path = path.replace('\\', "/");

                    let exists = project_index
                        .lines()
                        .any(|line| line.trim().replace('\\', "/") == normalized_path);

                    if !exists {
                        println!("⚠ Removing invalid planner task:\n{}", task);
            ...
            {
                println!("⚠ Planner proposed creating a new project. Rejecting task.");
...
Return ONLY valid JSON in this format:
  {{
     ...
     "#,
        ...
        let _clean_response = response.replace("\\", "\\\\");
        match serde_json::from_str::<Plan>(&response) {
            Ok(mut plan) => {
                self.validate_plan(&mut plan, project_index);
                println!("✅ Plan created with {} tasks.", plan.tasks.len());

                Some(plan)
            }

            Err(error) => {
                println!("❌ Failed to parse planner response: {}", error);
...
impl Agent for PlannerAgent {
    ...
    async fn execute(&self, _task: &Task) -> Option<ToolRequest> {
        println!("🧭 {} analyzing mission...", self.name());

        let tool_request = ToolRequest::InspectProject;

        println!("🧭 Planner requested tool: {:?}", tool_request);
