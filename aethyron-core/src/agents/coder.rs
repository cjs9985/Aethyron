...
impl CoderAgent {
    pub async fn execute_with_context(&self, task: &Task, context: &ProjectContext) -> CoderResult {
        ...
        loop {
            ...
            let change =
                match CodeGenerator::generate(&task.description, &context.project_index).await {
                    Ok(change) => change,
            ...
            if let Err(error) = self.validate_generated_path(&change.path, context) {
                println!("❌ {}", error);
            ...
            let result = if change.is_patch {
                EditorTool::append(&operation.path, &operation.content)
            } else if std::path::Path::new(&operation.path).exists() {
                println!("❌ Refusing to overwrite existing file: {}", operation.path);
                println!("⚠️ Existing-file patching is required for existing files.");
                break;
            } else {
                EditorTool::write(&operation.path, &operation.content)
            };
    ...
    fn validate_generated_path(&self, path: &str, context: &ProjectContext) -> Result<(), String> {
        ...
        let exists = context
            .files
            .iter()
            .any(|file| file.replace("\\", "/").ends_with(path));

        if !exists {
            return Err(format!("Target file does not exist: {}", path));
        }
